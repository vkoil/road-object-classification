...
training_transformer = transforms.Compose([transforms.RandomResizedCrop(32),
                                  transforms.RandomHorizontalFlip(),
                                  transforms.ToTensor(),
                                  transforms.Normalize((0.5, 0.5, 0.5),
                                                       (0.5, 0.5, 0.5))])  # For converting data into normalised tensors in the range [-1,1].
testing_transformer = transforms.Compose([  transforms.ToTensor(),
                                            transforms.Normalize((0.5, 0.5, 0.5),
                                                            (0.5, 0.5, 0.5))])  # For converting data into normalised tensors in the range [-1,1].
# Obtaining CIFAR-10 and CIFAR-100
training_set10 = torchvision.datasets.CIFAR10(root='./data', train=True, transform=training_transformer,
                                              download=False)  # change to true if required.
testing_set10 = torchvision.datasets.CIFAR10(root='./data', train=False, transform=testing_transformer, download=False)
training_set100 = torchvision.datasets.CIFAR100(root='./data', train=True, transform=training_transformer, download=False)
testing_set100 = torchvision.datasets.CIFAR100(root='./data', train=False, transform=testing_transformer, download=False)
...
class LeNet1A(nn.Module):
    def __init__(self):
        super(LeNet1A, self).__init__()
...
optimizerFunc = optim.Adam(model.parameters(), lr=learningRate, betas=(0.9, 0.99))
...
for epoch in range(Epoch):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(training_loader, 0):
        ...
        outputs = model(inputs)  # our LeNet1A output
